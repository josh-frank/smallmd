...
# ── 0b. Create deploy user if requested ──────
if [[ -n "$DEPLOY_USER" ]]; then
    ...
    # Add to www-data group so they can work alongside nginx/php-fpm
    usermod -aG www-data "$DEPLOY_USER"
    print_ok "Added $DEPLOY_USER to www-data group"

    # Scoped sudoers — only the three commands that truly need root
    SUDOERS_FILE="/etc/sudoers.d/smallmd-deploy"
    cat > "$SUDOERS_FILE" <<SUDOERS
# Generated by smallmd setup.sh
# Allows $DEPLOY_USER to run only the commands needed for smallmd deploys
$DEPLOY_USER ALL=(ALL) NOPASSWD: /usr/bin/apt-get, /bin/systemctl, /usr/sbin/nginx
SUDOERS
...
if [[ -n "$DEPLOY_USER" ]]; then
    # Deploy user owns the files; www-data can read via group
...
if [[ -n "$DEPLOY_USER" ]]; then
    ...
    echo "  To deploy      : su - $DEPLOY_USER"
    echo "  Sudoers        : /etc/sudoers.d/smallmd-deploy"
    echo ""
fi
